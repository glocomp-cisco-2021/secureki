...
'''
bot_email = 'seahackaton@webex.bot'
teams_token = ''
bot_url = 'http://<IPADDR>.ngrok.io'
bot_app_name = 'SeaHackaton'
appm_url = 'https://<IPADDR>:8443/OnceApproval?'
...
myapi = webex(access_token=teams_token)
...
def handle_cards(api, incoming_msg):
    ...
    print(sender.emails)
    # inject duo verify here
    if not checkduo.check_duo(sender.emails[0]): # assume first email is valid
        return "Action failed during MFA. Please check perform action from portal."
    if m["inputs"]["action"] == "reject":
        url = appm_url + m["inputs"]["target"]
        autoapprove.auto_browse(url)
        cmd = '''curl --request POST --url https://webexapis.com/v1/messages \
                --header 'Authorization: Bearer <ACCESS_TOKEN>' \
                --header 'Content-Type: application/json' -d '{"toPersonEmail": "<EMAIL_ADDR>","text": "Access application rejected by %s"}'
              ''' % '<APPROVER_NAME>'
        os.system(cmd)
        return "Request was rejected by {}".format('<APPROVER_NAME>')
        '''
        response = requests.get(url, verify=False)
        if response.status_code == 200:
            return "Request was rejected by {}".format(sender.firstName)
        else:
            return "Action failed server return code {}. Please check perform action from portal.".format(response.status_code)
        '''
    else:
        url = appm_url + m["inputs"]["target"]
        autoapprove.auto_browse(url)
        cmd = '''curl --request POST --url https://webexapis.com/v1/messages \
                --header 'Authorization: Bearer <ACCESS_TOKEN>' \
                --header 'Content-Type: application/json' -d '{"toPersonEmail": "<EMAIL_ADDR>","text": "Access application approved by %s"}'
              ''' % '<APPROVER_NAME>'
        os.system(cmd)
        return "Request was approved by {}".format('<APPROVER_NAME>')
        '''
        try:
            response = requests.get(url, verify=False)
            if response.status_code == 200:
                return "Request was approved by {}".format(sender.firstName)
            else:
                return "Action failed server return code {}. Please check perform action from portal.".format(response.status_code)
        except Exception as err:
            return "Action failed server return error: {}. Please check perform action from portal.".format(
                err)
        '''
